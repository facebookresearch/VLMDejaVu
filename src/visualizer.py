...
class Visualizer:
    ...
    def __init__(self, img_rgb, metadata=None, scale=1.0, instance_mode=ColorMode.IMAGE, colors=None):
        """
        Args:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        ##############
    # Show only one unique object per class
        if classes is not None:
            objs_filt, indx = [], []
            ...
            if boxes is not None:
                boxes = Boxes(boxes.tensor[indx])
            if keypoints is not None:
                keypoints = keypoints[indx]
            if masks is not None:
                masks_filt = []
                for i, mask in enumerate(masks):
                    if i in indx:
                        masks_filt.append(mask)
                masks = masks_filt
            labels = _create_text_labels(classes, scores, self.metadata.get("thing_classes", None))
        ##############
